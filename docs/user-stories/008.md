...
- Duplicate guard: enforce uniqueness on (email, meeting) at the DB level (unique constraint in 006's RSVP table) and check before insert; the API returns a distinct status (e.g. 409 Conflict) that the form surfaces as the "already registered" popup rather than a generic error
